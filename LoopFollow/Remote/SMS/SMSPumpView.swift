...
struct SMSPumpView: View {
    ...
    var body: some View {
        NavigationView {
            VStack(spacing: 20) {
                ...
                Form {
                    ...
                    Section {
                        ...
                        .disabled(isLoading || !SMSRemoteService.shared.validateSetup())
                    }
                }
            ...
            .navigationTitle("SMS Pump Control")
    ...
    private func sendPumpCommand() {
        ...
        Task {
            do {
                let success = try await SMSRemoteService.shared.sendPumpCommand(action: selectedAction, otp: otpCode)
                await MainActor.run {
                    isLoading = false
                    if success {
                        alertType = .success
