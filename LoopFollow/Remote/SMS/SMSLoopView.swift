...
struct SMSLoopView: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationTitle("SMS Loop Control")
    ...
    private func sendLoopCommand() {
        ...
        Task {
            do {
                let success = try await SMSRemoteService.shared.sendLoopCommand(action: selectedAction, otp: otpCode)
                await MainActor.run {
                    isLoading = false
                    if success {
                        alertType = .success
